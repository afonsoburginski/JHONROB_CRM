  ...
  const handleSaveContract = async () => {
    try {
      const contract = await prisma.contract.create({
        data: {
          product: {
            connect: { id: selectedProduct.value.id },
          },
          model: {
            connect: { id: selectedModel.value.id },
          },
          capacity: {
            connect: { id: selectedCapacity.value.id },
          },
          height: {
            connect: { id: selectedHeight.value.id },
          },
          power: selectedPower,
          // Adicione outras propriedades do contrato conforme necessário
        },
      });
    ...
    <form className='flex flex-col w-full max-w-screen-xl gap-5'>
      ...
      <div className='flex grid-cols-3 gap-5'>
        ...
        <div className='flex justify-end items-end grid-cols-3 w-full'>
          <button className="bg-white hover:bg-gray-100 text-gray-800 font-normal py-1 px-4 h-10 border rounded shadow" onClick={handleSaveContract}>
            Concluir
          </button>
        </div>
